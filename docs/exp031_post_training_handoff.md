...
The default policy is to keep the desktop local-probe branch isolated while VESSL training or Git publishing is active. After both training and the LOCAL campaign are complete, an explicit operator request may authorize report-only Git consolidation before the final EXP031 handoff. Such consolidation does not satisfy the handoff gate, promote EXP031, authorize official evaluation, or convert LOCAL evidence into official evidence. On 2026-07-11, the operator explicitly requested this consolidation before future tasks.
...
| EXP031 | 30 | training complete; final file-backed handoff pending |
...
The completed adaptive desktop campaign compared c4/ch16/s8 against c4/ch12/s8:
...
Current outcome: the gate failed because LOCAL_EXP034 did not beat LOCAL_EXP033 at seed 431 and its full-image regression exceeded the allowed component floor. The recorded decision is **do not promote c4/ch16/s8**.
...
Re-fetch before pushing. During active training, push only the isolated desktop branch and verify its remote SHA in a fresh detached worktree. After training completion, report-only consolidation requires explicit operator authorization, exact-tree review, preservation of pending-handoff labels, and detached remote verification. Branch consolidation never authorizes official evaluation or candidate promotion.
