...
def start() -> int:
    ...
    atomic_json(
        START,
        {
            "schema": "fastmri-r25-official-evaluation-start-v1",
            "state": "STARTED",
            "attempt": 1,
...
def finish(return_code: int) -> int:
    ...
    if (
        start_value.get("schema") != "fastmri-r25-official-evaluation-start-v1"
    ...
    if now > HARD_DEADLINE_UNIX:
        ...
        state = "FAIL"
    receipt = {
        "schema": "fastmri-r25-official-evaluation-receipt-v1",
        "state": state,
        "attempt": 1,
